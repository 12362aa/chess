...
const apkPath = process.argv[2] || path.join(__dirname, '..', 'chess-amkh-3.12.apk');
const assetName = 'chess-amkh-3.12.apk';
const tag = 'v3.12';
const releaseName = 'شطرنج Am-Kh — شاشة ترحيب عند أول تشغيل وحضور أدقّ للأصدقاء';
const releaseBody = [
  '♟ **شطرنج Am-Kh** — تحديث جديد (بناء 28)',
  '',
  '- 👋 **شاشة ترحيب عند أول تشغيل** — تعرّفك بما يمنحه الحساب: أصدقاء ومباريات مصنّفة ومحادثات ومكالمات ومزامنة للتقدّم على أي جهاز. القرار صريح، ومعه خيار **المتابعة دون حساب** لأن اللعب أمام الحاسوب يعمل كاملًا دون تسجيل. تظهر مرّة واحدة، ثم لا تعود.',
  '- 🟢 **حضور الأصدقاء يُحدَّث فور عودة الاتصال** — لم يبقَ الصديق المتصل ظاهرًا غائبًا بعد انقطاع قصير في الشبكة.',
  '- 👁 **شارة المشاهدة تختفي بانتهاء المباراة** — كانت تبقى معلّقة فتؤدّي الضغطة إلى طريق مسدود.',
  '- 🏆 **لوحة الصدارة تُفتح فوق المحادثة** لا تحتها، فلم تعد تُحجب بها.',
  '- ⌨️ **لوحة مفاتيح ومربّع كتابة مستقرّان في المحادثة على الأجهزة اللوحية** — الترويسة تبقى ثابتة، ومربّع الكتابة يقف فوق لوحة المفاتيح مباشرة دون انقراض.',
  '- 🔍 **المباراة المُشاهَدة بملء الشاشة برقعة أكبر** — متابعة أوضح لمباراة صديقك.',
  '- 📞 **إشارات المكالمات والدعوات لا تُفقد** — انتهت حالة الرنّة التي لا تصل، وحالة قبول الدعوة الذي يضيع في الصمت دون أن تبدأ المباراة.',
  '',
  'رابط التنزيل الدائم: https://github.com/12362aa/chess/releases/download/v3.12/chess-amkh-3.12.apk',
].join('\n');
