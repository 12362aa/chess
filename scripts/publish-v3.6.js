/* نشر نسخة APK على GitHub Releases (v3.6).
   نفس نمط scripts/publish-v3.5.js: التوكن من .env (GH_TOKEN) مش مكتوب في
   الملف، فالسكربت آمن للتتبّع في git.

   node scripts/publish-v3.6.js <path-to-apk>
*/
const fs = require('fs');
...
if (!apkPath || !fs.existsSync(apkPath)) {
  console.error('لازم مسار APK موجود. الاستخدام: node scripts/publish-v3.6.js <apk>');
  process.exit(1);
}

const tag = 'v3.6';
const releaseName = 'شطرنج Am-Kh v3.6 — إعدادات الخصوصية ودعوات الحفلات';
const releaseBody = [
  '## ♟ تحديث الخصوصية',
  '',
  '### الجديد',
  '- **إعدادات خصوصية على طراز واتساب** — قسم جديد في «الحساب» بيتحكّم مين يقدر يضيفك لحفلة، يبعتلك رسالة، يبعتلك دعوة لعب، يبعتلك طلب صداقة، ومين يشوف حالتك (متصل/آخر ظهور) وصورتك وتقييمك. الاختيارات: الكل / الأصدقاء فقط / لا أحد. الإعدادات بتتحفظ على حسابك على الخادم وبتفضل ثابتة لحد ما تغيّرها بنفسك.',
  '- **الإضافة للحفلة بقت دعوة، مش إضافة إجبارية** — لو خصوصية الشخص مابتسمحش بالإضافة المباشرة، بيتبعتله دعوة معلّقة (بتنتهي بعد ٧٢ ساعة) بدل ما يتضاف من غير إذنه. بيوصله كارت فيه صوت وثيم يقبل أو يرفض.',
  '- **إخفاء التقييم/الحالة/الصورة** بيتطبّق فعليًا في البحث وقوائم الأصدقاء حسب إعداداتك، مش في الشكل بس.',
  '- الحظر بيغلب أي إعداد — المحظور مايقدرش يضيفك ولا يبعتلك أي حاجة.',
  '',
  '### التثبيت',
...
(async () => {
  ...
  const assetName = 'Chess-AmKh-v3.6.apk';
