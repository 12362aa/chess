/* نشر نسخة APK على GitHub Releases (v3.3).
   نفس نمط scripts/publish-v3.2.js: التوكن من .env (GH_TOKEN) مش مكتوب في
   الملف، فالسكربت آمن للتتبّع في git.

   node scripts/publish-v3.3.js <path-to-apk>
*/
const fs = require('fs');
...
if (!apkPath || !fs.existsSync(apkPath)) {
  console.error('لازم مسار APK موجود. الاستخدام: node scripts/publish-v3.3.js <apk>');
  process.exit(1);
}

const tag = 'v3.3';
const releaseName = 'شطرنج Am-Kh v3.3 — الإشعارات اشتغلت، وشات بيتبع الثيم';
const releaseBody = [
  '## ♟ تحديث الإشعارات والشات',
  '',
  '### الجديد والمُصلَّح',
  '- **الإشعارات بقت شغّالة** — اتصلّح إعداد الخادم اللي كان مانع وصول أي تنبيه.',
  '- **الشات بقى بيتبع ثيم التطبيق** — الألوان بتتغيّر مع الثيم اللي مختاره، مش ثابتة.',
  '- **أيقونات مرسومة** بدل الإيموجي في تشغيل/إيقاف الرسائل الصوتية بشات المباراة.',
  '- **تشغيل الرسائل الصوتية بقى أثبت** — مفيش تشغيل مزدوج ولا استمرار بعد الإيقاف.',
  '',
  '### الحفلات (الشات الجماعي)',
  '- **صورة للحفلة** — أيقونة افتراضية أنيقة، والمالك يقدر يرفع صورة للحفلة.',
  '- **التسمية بقت «حفلة شطرنجية»** في كل مكان.',
  '- **النوافذ بتظهر في مكانها الصح** جوه شاشة الأصدقاء/الحفلات مش برّه.',
  '',
  '### التثبيت',
...
(async () => {
  ...
  const assetName = 'Chess-AmKh-v3.3.apk';
