...
const menuTarget = `<div class="mc" data-mode="local" onclick="if(typeof SFX !== 'undefined') SFX.btn(); G.startLocalGame()">
      <span class="mc-tag">LOCAL</span>
      <span class="mc-ic ic-green">⚔</span>
      <div class="mc-t">لاعبَان</div>
      <div class="mc-d">على نفس الشاشة بالتناوب</div>
      <span class="mc-b bv">أوفلاين ◈</span>
    </div>
    <div class="mc" data-mode="settings" onclick="if(typeof SFX !== 'undefined') SFX.btn(); Nav.settings()">`;

const menuReplacement = `<div class="mc" data-mode="local" onclick="if(typeof SFX !== 'undefined') SFX.btn(); G.startLocalGame()">
      <span class="mc-tag">LOCAL</span>
      <span class="mc-ic ic-green">⚔</span>
      <div class="mc-t">لاعبَان</div>
      <div class="mc-d">على نفس الشاشة بالتناوب</div>
    ...
    <div class="mc" data-mode="bluetooth" onclick="if(typeof SFX !== 'undefined') SFX.btn(); Nav.bluetooth()">
...
const screenTarget = `<button class="btnb" onclick="if(typeof SFX !== 'undefined') SFX.btn();Nav.menu()">← العودة</button>
  </div>
</div>

<!-- GAME -->`;

const screenReplacement = `<button class="btnb" onclick="if(typeof SFX !== 'undefined') SFX.btn();Nav.menu()">← العودة</button>
  </div>
</div>
...
// 3. Add Nav.bluetooth
const navTarget = `online(){Nav.show('s-online');try{OL.init();}catch(e){}},
  local(){Nav.show('s-local');},`;

const navReplacement = `online(){Nav.show('s-online');try{OL.init();}catch(e){}},
  bluetooth(){Nav.show('s-bluetooth');},
  local(){Nav.show('s-local');},`;

html = html.replace(navTarget, navReplacement);

// 4. Add Logic at the end
const scriptTarget = `window.addEventListener('beforeunload',()=>{
  try{CHAT._stopRecording();}catch(e){}
});
</script>`;

const scriptReplacement = `window.addEventListener('beforeunload',()=>{
  try{CHAT._stopRecording();}catch(e){}
});
  ...
  async hostGame() {
    ...
    try {
      ...
      if (BleClient.startAdvertising) {
          ...
          this.setStatus("وضع الاستضافة غير مدعوم في هذا الإصدار بشكل كامل كـ Peripheral. جرب تطبيق طرف ثالث لعمل advertise.");
      }
    } catch(e) {
